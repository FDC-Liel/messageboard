        ...
        <div class="card">
            ...
            <div class="card-body" id="pagination">
                <?php foreach($messages as $message):?>
                    <?php if($current_user['id'] != $message['Message']['user_id']):?>
                        <div class="row recipient-box d-flex flex-row mb-2">
                            <div class="d-flex" id="recipient-box">
                                <?php 
                                    $img = isset($message['User']['image']) ? $message['User']['image'] : 'default-avatar.jpg';
                                ...
                                <div class="media-body border border-dark mr-2">
                                    ...
                                    <?php endif;?>
                                    <hr class="text-dark">
                                    <p class="text-right text-black-50 mx-2" style="font-size: 8px"><?php echo date('Y/m/d h:i',strtotime($message['Message']['created']));?></p>
                                </div>
                            </div>
                        </div>
                    <?php else:?>
                        <div class="row sender-box d-flex flex-row-reverse mb-2">
                            <div class="d-flex flex-row-reverse" id="sender-box">
...
<script>
    $(document).ready(function(){
        ...
        function loadMoreMessage() {
            ...
            $.ajax({
                url: '<?php echo $this->Html->url(['controller' => 'messages', 'action' => 'loadMoreMessage']); ?>',
                type: 'post',
                dataType: 'json',
                data: {
                    page: page, // Send the current page
                    //conversationId: '<?php //echo $conversation_id['Conversation']['id']; ?>'
                },
                success: function (data) {
                    ...
                    if (data.length > 0) {
                        ...
                        //var userId = '<?php //echo $messages['User']['id']; ?>';
                                ...
                                var messageHtml = `
                                    <div class="row sender-box d-flex flex-row-reverse mb-2">
                                        <div class="media">
                                            ...
                                            <img src=" `+ baseUrl +`/`+ message.User.img +`" alt="Default Avatar" class="ml-2 mr-2 rounded-circle " w="64" h="64">
                                        </div>
                                    </div>
                                `;
                                messageContainer.append(messageHtml);
        ...
        // Load more message when the "Show More" button is clicked
        $('#show-more').on('click', function () {
            loadMoreMessage();
        });

        // Load more message when the user scrolls to the bottom of the message container
        ...
        $('#messageAdd').on('submit',function(e){
            ...
            $.ajax({
                url: '<?php echo $this->Html->url(array('controller' => 'messages', 'action' => 'addMessage', 'id' => $message['Conversation']['id'])); ?>',
                type: 'post',
                dataType: 'json',
                data: requestData,
                success: function (data)
                    ...
                    var newMessageHtml = `
                        <div class="row sender-box d-flex flex-row-reverse mb-2">
                            <div class="row" id="sender-box">
                                ...
                                <div class="media-body border border-dark ml-2">
                                    <p class="ml-1 mr-1 text-justify">`+ data.Message.message +`</p>
                                    <hr class="text-dark">
        ...
        $(document).on('click', '.delete-all', function() {
            var conversationId = '<?php echo $message['Conversation']['id'];?>';
            var baseUrl = '<?php echo $this->Html->url(array('controller' => 'messages', 'action' => 'deleteMessageinConversation'), true); ?>';
